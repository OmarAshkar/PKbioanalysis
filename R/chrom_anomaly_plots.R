...
.vis_data <- function(chrom_res){
   dat <- chrom_res@peaks |>
        dplyr::select("sample_id", "filename", "area", "compound_id", "observed_peak_start", "observed_peak_end", "observed_rt") |>
        dplyr::mutate(sample_id = as.numeric(sample_id)) |>
        # get compount name
        dplyr::left_join(chrom_res@compounds |> select("compound_id", "compound", "transition_id") , by = "compound_id")  |>
        # get type
        dplyr::left_join(chrom_res@metadata |> select("filename", "type"), by = "filename") |>
        # get q3 from transitons tab
        dplyr::left_join(chrom_res@transitions |> select("transition_id", "q3"), by = "transition_id")  |>
...
plot_RT.ChromRes <- function(chrom_res){
    ...
    res <- ggplot2::ggplot(dat, aes(y = sample_id, x = observed_rt, color = type)) +
        ggiraph::geom_point_interactive(aes(tooltip = split_id,
            data_id = split_id),
            stat = "identity", size = 0.7) +
        ggplot2::theme_minimal() +
        ggiraph::geom_errorbarh_interactive(aes(
            data_id = split_id,
            tooltip = compound_id,
            xmin = observed_peak_start, xmax = observed_peak_end), height = 0.1) +
        ggplot2::theme(legend.position = "none") +
        ggplot2::labs(x = "Peak Area", y = "") +
        ggplot2::facet_wrap(compound_label~., drop = FALSE, ncol = 6, scales = "free") +
...
plot_area_bar.ChromRes <- function(chrom_res, log_scale = TRUE){
    ...
    res <- ggplot2::ggplot(dat, aes(x = sample_id, y = area, fill = type)) +
        ggiraph::geom_bar_interactive(aes(tooltip = paste0(filename, area, type), 
                data_id = split_id),
                color = "white",
                                      stat = "identity",
        ...
        theme(axis.text.y = ggplot2::element_text(angle = 10, size = 4),
...
plot_area_dot.ChromRes <- function(chrom_res, log_scale = TRUE){
    ...
    res <- ggplot2::ggplot(dat, aes(y = type, x = area))+
        ggiraph::geom_point_interactive(aes(tooltip = paste0(filename, area, type),
                                            data_id =  split_id),
                                        shape = 21, size = 0.7, fill = "green") +
        ggplot2::theme_minimal() +
        ggplot2::theme(legend.position = "bottom") +
        ggplot2::labs(x = "Peak Area", y = "") +
        ggiraph::facet_wrap_interactive(~compound_label, scales = "free_x", ncol = 4) +
        theme(axis.text.y = ggplot2::element_text(angle = 15, size = 5),
