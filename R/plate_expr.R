...
undo_last_call <- function(prev) {
    if (!is.call(prev)) stop("Not a call")
    inner <- prev[[2]]
    if (!is.call(prev)) stop("Inner is not a call, nothing more to do")
    inner
}
...
print_expr <- function(expr) {
  ...
  for (i in rev(seq_along(parts))) {
    ...
    if (length(args) > 0) {
      arg_strs <- mapply(function(arg, nm) {
        if (nm == "") {
          deparse1(arg)
        } else {
          paste0(nm, " = ", deparse1(arg))
        }
      }, args, names(args), SIMPLIFY = TRUE)
