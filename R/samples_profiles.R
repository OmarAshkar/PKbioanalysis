  ...
  for (i in chrom_res@compounds$compound_id) {
    ...
    if (has_linearity(chrom_res, i)) {
      samples_conc[[i]] <- chrom_res@linearity[[i]]$linearitytab |>
        dplyr::filter(
          type == "Sample" & !is.na(sampling_time) & !is.na(subject_id)
...
plot_precision <- function(chrom_res, compound_id = NULL) {
  df <- names(chrom_res@linearity) |>
    lapply(\(x) {
      chrom_res@linearity[[x]]$linearitytab |> dplyr::mutate(compound_id = x)
    }) |>
    dplyr::bind_rows() |>
    dplyr::filter(type == "QC")

  df2 <- df |>
    group_by(.data$compound_id, .data$actual_conc) |>
    dplyr::summarize(
      sd = sd(estimated_conc),
      cv = sd(estimated_conc) / mean(estimated_conc) * 100
    )
  ...
  patchwork::wrap_plots(x, y, ncol = 2)
}
