...
integerate_ai <- function(chrom_res, transition_id, sample_id, peak_start, peak_end){
  ...
  prompt <- paste(
      "What is the observed retention time, peak start and peak end for this chromatogram? Look for the between peak signal roughly between ",  peak_start,
      "and ", peak_end, " minutes respectively.",
...
studydesign_ai <- function(chat, study_id) {
  ...
  prompt <- paste(
    ...
    "Note study subject type is ", get_study_subject_type(study_id),
...
ai_chat_module_ui <- function(id, title = "AI Assistant") {
  ns <- NS(id)
  shinyWidgets::actionBttn(ns("invoke_btn"), paste("Open", title), icon = icon("robot"), 
...
ai_chat_module_server <- function(id, chatfunc, response_function, response_args, botname ) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$invoke_btn, {
      ...
      tryCatch({
        ...
        showNotification(paste("Error invoking AI:", e$message), type = "error")
      })
    })
