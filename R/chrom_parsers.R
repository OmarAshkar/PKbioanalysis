...
.parsewatersraw <- function(dir) {
  ...
  py <- reticulate::import_from_path("src", path = system.file("pysrc", package="PKbioanalysis"))
  ...
  rawreadr <- function(file) {
    ...
    colnames(f[[1]]) <- c(
      transition_id, coln[(length(transition_id)+1):length(coln)]
    )
    ...
    dat <- f[[1]] |> 
      tidyr::pivot_longer(cols = !!transition_id, # vector of columns to sepecify the transitions' intensity 
        names_to = "transition", values_to = "Intensity")
  ...
  failed <- sample_transitions |> sapply(\(x) identical(x, sample_transitions[[1]]))   
  if(any(!failed)){
    failure_indices <- which(!failed)
    failure_table <- data.frame(Index = basename(x[failure_indices]), Status = "Failure")
    print(failure_table) 
    stopifnot("Please remove inconsistent methods") # all transitions must be same across all samples
  }
  
  sample_transitions <- sample_transitions |> dplyr::bind_rows() |> dplyr::distinct() 
  ## Must assert all transitions are same across all samples

  #need only RT and T1 - Tn. Others will be sub list called meta
  df_list <- split(df, df$sample) |> lapply(\(x) {
    peakdf <- x[, c("RT", "transition", "Intensity")] |> tidyr::pivot_wider(names_from = "transition", values_from = "Intensity")
    metadf <- x[, !colnames(x) %in% c("RT", "transition", "transition_id", "Intensity")] |>
       dplyr::distinct() |> 
...
.parsemzml <- function(dir){
  ...
  res <- lapply(files, \(x) {
    ...
    y <- x$chroms |> 
      ...
      dplyr::mutate(q1 = gsub(chrom_type, pattern = ".*Q1=(.*)\\sQ3=(.*)\\sfunction.*", replacement = "\\1"),
                    q3 = gsub(chrom_type, pattern = ".*Q1=(.*)\\sQ3=(.*)\\sfunction.*", replacement = "\\2")) |> 
    ...
    dat <- y |> select(RT, transition_id, Intensity) |>
         tidyr::pivot_wider(names_from = "transition_id", values_from = "Intensity") 

    list(sample_chrom = dat, 
      sample_metadata = data.frame(filename = tools::file_path_sans_ext(unique(y$filename)), 
        vendor = "mzML", date = x$metadata$timestamp),
      sample_transitions =  y |> dplyr::select(transition_id, q1, q3) |> dplyr::distinct()
      )
...
.sort_chromatograms <- function(chroms_list){
  ...
  chroms_list <- chroms_list[order(sapply(chroms_list, function(x) x$sample_metadata$date))]
...
.check_transitions <- function(chrom_list){
  ...
  if(!all(sapply(x, identical, x = x[[1]]))){
    stop("Transitions are not same across samples. Please ensure all chromatograms have same transitions")
  }
...
#' @noRd
.construct_experiment_transitions <- function(transition_df, transitiondb, diff_tol = 0.5){
  checkmate::assertDataFrame(transition_df)
  checkmate::assertNames(colnames(transition_df), must.include = c("q1", "q3", "transition_id"))
  checkmate::assertDataFrame(transitiondb, null.ok = FALSE)
  checkmate::assertNames(colnames(transitiondb), must.include = c("q1", "q3", "transition_id", "method_id"))
  ...
  if(any(abs(transition_df$q1 - transitiondb$q1) > diff_tol) ||
    ...
    print(data.frame(
      ...
      mismatch = abs(transition_df$q1 - transitiondb$q1) > diff_tol | abs(transition_df$q3 - transitiondb$q3) > diff_tol
    ))
    stop("Transition values do not match between chromatography and method database.")
  }
...
.construct_experiment_peaktab <- function(chrom_res){
    ...
    compounds_ids <- chrom_res$exp_compounds |> dplyr::pull(compound_id) |> unique()
    samples_ids <- chrom_res$metadata |> dplyr::pull(sample_id) |> unique()

    chrom_res$exp_peaktab <- expand.grid(sample_id = samples_ids, compound_id = compounds_ids) |> 
    ...
    chrom_res$exp_peaktab <- chrom_res$exp_peaktab |> 
      dplyr::left_join(chrom_res$metadata |> dplyr::select(sample_id, filename), by = c("sample_id" = "sample_id")) |>
      dplyr::mutate(filename = tools::file_path_sans_ext(filename)) |>  # remove any file extensions 
      dplyr::select("sample_id","filename","compound_id","area",
        "observed_peak_start","observed_peak_end","observed_peak_height","observed_rt","manual")
...
.construct_experiment_peaktab.deprecated <- function(res, peaks){
  ...
  stopifnot(all(c("filename", "peak_start", "peak_end", "compound", "q1", "q3") %in% colnames(peaks)))
  ...
  peaks <- peaks |> dplyr::left_join(res$metadata |> dplyr::select(filename, sample_id),
    by = c("filename" = "filename")) 
  
  # assert all chromatograms has a sample_id
  names(res$runs$files) <- res$metadata$sample_id
  stopifnot(length(unique(names(res$runs$files))) == length(res$metadata$sample_id)) # all samples must be unique

  # within all samples (groups), assert equal number of compounds 
  check <- peaks |> dplyr::group_by(filename) |> 
    dplyr::summarise(n_cmpd = n_distinct(compound), n_q1 = n_distinct(q1), n_q3 = n_distinct(q3)) |>
    select(-"filename") |>
    dplyr::distinct() 
  ...
  peaks <- peaks |> 
    dplyr::rename("observed_peak_start" = peak_start, "observed_peak_end" = peak_end) |> 
    dplyr::mutate(observed_peak_start = as.numeric(observed_peak_start), observed_peak_end = as.numeric(observed_peak_end)) |>
    dplyr::mutate(observed_rt = (observed_peak_start + observed_peak_end) / 2) |>
    ...
    dplyr::left_join(res$exp_transitions |> 
      dplyr::mutate(dum_q1 = round (q1, 0), dum_q3 = round(q3, 0))
    , by = c("dum_q1", "dum_q3"))  |> 
...
.construct_experiment_compounds <- function(method_id, transition_df){
  ...
  tryCatch({
    compare1 <- transition_df |> dplyr::select("transition_id", "q1", "q3") |> distinct()
    compare2 <- compoundsdf |> dplyr::select("transition_id", "q1", "q3") |> distinct()
  ...
  }, error = function(e) { # elaborative error messages. Print both 
      print(transition_df |> dplyr::select( "transition_id", "q1", "q3") |> distinct())
      print(compoundsdf |> dplyr::select( "transition_id", "q1", "q3") |> distinct())
      print(e)
  ...
  exp_compounds <- compoundsdf |>
    select("compound_id", "compound", "transition_id", "expected_peak_start", 
      "expected_peak_end", "expected_rt", "IS_id")
...
.write_transitions <- function(chrom_res){
  ...
  for(trans_row in 1:nrow(new_chrom_res$exp_transitions)){
    ...
    max_id <- tbl(db, "methodstab") |> 
      ...
      as.data.frame() |> pull(id)
    max_id <- ifelse(is.na(max_id), 0, max_id)

    query <- tbl(db, "methodstab") |> 
      dplyr::filter(q1 == q1, q3 == q3, inlet_method == inlet_method) |> as.data.frame()
    
    
    old_transition_id <-  new_chrom_res$exp_transitions[trans_row, "transition_id"]
    
    if(nrow(query) == 0){
      ## add to db and retrieve the id 
      dbWriteTable(db, "methodstab", 
        data.frame(transition_id = max_id +1, q1 = q1, q3 = q3, method_gradient = inlet_method, transition_label = NA), 
    ...
    } else if(nrow(query) == 1){ # update the transition id and labels

      new_transition_id <- query |> dplyr::pull(transition_id) |> as.character() 
      transition_label <- query |> dplyr::pull(transition_label) |> as.character() 
    }
    
    new_chrom_res$exp_transitions$transition_id <- ifelse(new_chrom_res$exp_transitions$transition_id == old_transition_id, 
      new_transition_id, 
    ...
    new_chrom_res$runs <- lapply(new_chrom_res$runs, \(x) {
      colnames(x$sample_chrom) <- ifelse(colnames(x$sample_chrom) == old_transition_id, 
    ...
    new_chrom_res$exp_transitions$transition_label <- ifelse(new_chrom_res$exp_transitions$transition_id == new_transition_id, 
      transition_label, 
