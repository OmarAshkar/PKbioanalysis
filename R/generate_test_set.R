...
.generate_testing_injeseq <- function(){
    ...
    names <- system.file("extdata", "waters_MZML_ex", package="PKChromaMetrics") |> 
        list.files() |> 
        basename() 
    ...
    q1 <- c(104.02, 104.02,  # GABA
        112.04, 112.04, # histamine
        146.07, 146.07, # acetylcholine
        148.03, 148.03, # glutamate
        154.07, 154.07,  # domapine
        158.07,  # dopamine d4
        168.05, 168.05,  # 3-methoxytyramine
        170.06, 170.06, # norepinephrine
        177.1, 177.1, # serotonin
        184.1, 184.1,  # epinephrine
        192.08, 192.08) # 5-HIAA
    q3 <- c(42.6, 68.9, # GABA
        67.9, 82.9,  # histamine
        59.99, 86.99, # acetylcholine
        84.13, 102.13, # glutamate
        91.1, 118.9, # domapine
        94.7,  # dopamine d4
        91.1, 118.9,   # 3-methoxytyramine
        106.9, 134.8,  # norepinephrine
        105.02, 115.03, # serotonin
        107.02, 134.9,  # epinephrine
        118.19, 146.04) # 5-HIAA
    trans <- data.frame(q1, q3, compound = c(
        "GABA", "GABA",
        "histamine", "histamine", 
        ...
        "dopamine", "dopamine",
         "dopamine d4", 
         "3-methoxytyramine", "3-methoxytyramine", 
