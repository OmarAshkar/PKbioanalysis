...
.onLoad <- function(libname, pkgname) {
  ...
  options(PKbioanalysis.cache_dir = file.path(options("PKbioanalysis.data_dir"), "plates_cache"))
  ...
  if (dir.exists(pysrc_path)) {
    py <<- reticulate::import_from_path(module = "src", path = pysrc_path, delay_load = TRUE)
