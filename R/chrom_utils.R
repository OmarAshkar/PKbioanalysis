...
get_compound_name <- function(chrom_res, compound_id) {
    compounds <- chrom_res@compounds[chrom_res@compounds$compound_id == compound_id, "compound"]
    compounds
}
...
get_compound_ID <- function(chrom_res, compound_name) {
    compound_id <- chrom_res@compounds[chrom_res@compounds$compound == compound_name, "compound_id"]
...
update_IS <- function(chrom_res, compound_id, standard_id){
    ...
    chrom_res@compounds <- chrom_res@compounds |>
        dplyr::mutate(IS_id = ifelse(.data$compound_id == !!compound_id, standard_id, IS_id))
    chrom_res
...
get_cmpd_IS <- function(chrom_res, compound_id) {
    ...
    is_id <- chrom_res@compounds |> 
        dplyr::filter(.data$compound_id == !!compound_id) |> pull("IS_id")
...
.compound_trans_df <- function(chrom_res) {
    ...
    cmpds <- dplyr::left_join(cmpds, chrom_res@transitions, by = "transition_id") |> 
...
#' @noRd
.get_compound_id_from_compound_trans <- function(cmpd_trans_df, compound_trans){
...
chromres_to_matrix <- function(chrom_res, wide = FALSE) {
     x <- chrom_res@peaks |> 
        dplyr::left_join(.compound_trans_df(chrom_res) |> 
            select("compound_id", "compound_trans"), by = "compound_id") |>
...
#' }
setGeneric("is_integrated", function(chrom_res, compound_id, sample_id = NULL) standardGeneric("is_integrated")) 

setMethod("is_integrated", signature(chrom_res = "ChromRes"), 
    function(chrom_res, compound_id, sample_id = NULL) is_integrated_chrom_res(chrom_res, compound_id, sample_id))

setMethod("is_integrated", signature(chrom_res = "ChromResBase"), 
    function(chrom_res, compound_id, sample_id = NULL) is_integrated_chrom_base(chrom_res))
...
is_integrated_chrom_res <- function(chrom_res, compound_id,sample_id = NULL){
    ...
    # } else {
    res <- peaktab |>
        dplyr::filter(.data$sample_id == sample_id_filter & .data$compound_id == compound_id_filter) |>
        dplyr::select(
            observed_rt,
...
has_default_RT <- function(chrom_res, compound_id){
    ...
    if(!(compound_id_filter %in% compounds$compound_id)){
        ...
        res <- compounds |>
            ...
            dplyr::select("expected_peak_start", "expected_peak_end") |> complete.cases()
    }
    res
}
...
update_metadata <- function(chrom_res, metadata, ignore_unmatched = TRUE){
    ...
    # assert type clean 
    stopifnot(metadata$type  %in% c("Sample", "QC", "Blank", "Standard", "DoubleBlank", "Suitability", "", NA))
