...
#' @noRd
setClass("ChromRes", contains = "ChromResBase", slots = c(
    runs = "list"
))
...
check_chromatograms_consistency <- function(object){
    ...
    if(nrow(object@peaks) != length(object@runs$files) * nrow(object@compounds)) {
    ...
    if(!identical(unique(names(object@runs$files)), unique(object@peaks$sample_id))) {
...
check_all <- function(object){
    ...
    if(nrow(object@peaks) != (nrow(object@compounds) * length(unique(object@peaks$sample_id)))){
        stop("Number of peaks does not match the number of samples and compounds.")
    }
    ...
    # peaktab columns 
    checkmate::assertNames(names(object@peaks), 
        identical.to  = c(
            "sample_id", "filename", "compound_id", "area", 
            "observed_peak_start", "observed_peak_end",
            "observed_peak_height", "observed_rt", "manual"))
    ...
    checkmate::assertNames(names(object@transitions), 
        identical.to  =  c("transition_id", "transition_label", "q1", "q3", "method_id"))
    ...
    checkmate::assertNames(names(object@compounds),
        permutation.of = c("compound_id", "compound",
        "transition_id",
        "expected_peak_start", "expected_peak_end", "expected_rt", "IS_id"))
...
read_chrom <- function(dir, format = "waters_raw", method) {
    ...
    transitions_df <- .construct_experiment_transitions(chroms_list[[1]]$sample_transitions, transitionsdb)

    compounds_df <- .construct_experiment_compounds(method, transitions_df)

    res <- list(runs = list(files = chroms_list),
...
plot_chrom <- function(chrom_res, ncol = 2, transitions_ids = NULL, 
    sample_id, integrated = FALSE, show_RT= FALSE, smoothed = FALSE) {
    ...
    chrom_res <- filter_chrom(chrom_res, 
        transitions_ids = transitions_ids, samples_id = sample_id)
    ...
    transitions <- chrom_res@transitions |> mutate(transition_id = paste0("T", transition_id)) 
    ...
    active_df <- dat[[1]][[yvar]] |> 
        tidyr::pivot_longer(cols = -c("RT"), names_to = "transition_id", values_to = "Intensity") |>
        dplyr::mutate(sample_id = as.numeric(chrom_res@metadata$sample_id),
    ...
    active_df <- active_df |>
        ...
        left_join(peaktab,
            by = c("sample_id" , "compound_id")) # get the RT markers


    sample_name <- unique(active_df$filename)
    caption <- unique(active_df$date_captured)
    if (ncol > 0) {
        y <- ggplot( active_df, aes(x = RT, y = Intensity, color = transition_label)) +
            geom_line() +
            theme_minimal() +
            theme(legend.position = "none") +
            labs(subtitle = sample_name, x = "RT (Minutes)", y = "Intensity", caption = caption) +
    ...
    if(show_RT){
        y <- y + ggplot2::geom_point(aes(x = observed_rt, y =  observed_peak_height ), color = "purple") +
            ggplot2::geom_point(aes(x = observed_peak_start, y = 0), color = "blue") +
            ggplot2::geom_point(aes(x = observed_peak_end, y = 0), color = "green") +
            ggplot2::geom_text(aes(x = observed_rt, y = observed_peak_height, 
                label = paste0("RT:", observed_rt %>% round(2), " Area: ", round(area, 3))), 
                check_overlap = T, hjust = 0.5, vjust = 2)
    }
    if(integrated){
        # add peak area and cmpd name 
        cmpd_lab <- sapply(active_df$compound_id, \(x) get_compound_name(chrom_res, x))
        y = y+
            ggplot2::geom_area(mapping = aes(x = ifelse(RT>=observed_peak_start & RT<= observed_peak_end , RT, NA)), 
                fill = "grey", alpha = 0.5) +
            ggplot2::geom_text(mapping = aes(x = observed_rt, 
                y = observed_peak_height, label = cmpd_lab),
                check_overlap = T, hjust = -0.5, vjust = 0)
    }
...
#' @import checkmate
#' @export
filter_chrom <- function(chrom_res, transitions_ids = NULL, samples_id = NULL, cmpd_ids = NULL) {
    checkmate::assertClass(chrom_res, "ChromRes")
    ...
    exp_peaktab <- exp_peaktab |> 
        dplyr::filter(.data$sample_id %in% samples_id & .data$compound_id %in% exp_compounds$compound_id) 

    exp_transitions <- exp_transitions |> dplyr::filter(.data$transition_id %in% transitions_ids)
...
smooth_chrom <- function(chrom_res,  filter = "mean", window = 2, iter = 2) {
    ...
    checkmate::assert_choice(filter, choices = c("mean", "median", "savgol", "gaussian"))
    ...
    dat <- lapply(dat, \(x){
        x$smoothed <- x[[1]] |> 
            dplyr::mutate(across(-RT, \(x) smooth_peaks(x, filter, 
...
print.ChromRes <- function(x) {
    ...
    cat("Number of transitions: ", length(unique(x@transitions$transition_id)), "\n")
