...
upload_quant_file_ui <- function(id){
  ...
  bslib::page_fillable(
    bslib::layout_columns(
      fileInput("quant_file", "Upload Quant File", accept = c(".csv", ".txt", ".xml")),

      selectInput("upload_format", "Select Method", 
        choices = c("TargetLynx XML" = "targetlynx_xml", "TargetLynx CSV" = "targetlynx_csv", "General CSV/TXT" = "generic"), 
        selected = "targetlynx_csv"),

      selectInput(ns("method_id"), "Select Method ID", choices = setNames(.get_methodsdb()$method_id, .get_methodsdb()$method))
    ),
...
quant_ui <- function(id){
  ...
  bslib::page_fillable(
    bslib::navset_underline(
      bslib::nav_panel(title = "Table", reactable::reactableOutput(ns("quant_table"))),
    )
  )
}
...
linearity_data_server <- function(id, quantres, cmpd_trans_df){
  # stopifnot(is.reactive(chrom_res))
  moduleServer(id, function(input, output, session) {
    ...
    output$linearity_table <- renderDT({
        ...
        quantres()@linearity[[cmpd_id()]]$linearitytab  |>
          select("filename", "type", "include", "abs_response", "rel_response",
            "stdconc", "estimated_conc", "residual_conc", "dev_conc", "passed") |>
          DT::datatable(
    ...
    observeEvent(input$run_linearity_btn, {
      req(cmpd_id())
      tryCatch({
        run_linearity(quantres(), cmpd_id(), 
          weight = input$weight, 
          model = input$model, 
    ...
    observeEvent(input$exclude_cs_btn, {
      ...
      exclude_linearity(quantres(), cmpd_id(), last_selected_points()) |> quantres()
      tryCatch({
        run_linearity(quantres(), cmpd_id(), input$weight, input$model, 
    ...
    observeEvent(input$include_cs_btn, {
      ...
      include_linearity(quantres(), cmpd_id(), last_selected_points()) |> quantres()
                ...
                bslib::layout_columns(
                  ...
                  bslib::card(
                    ...
                    bslib::navset_tab(
                      ...
                      bslib::nav_panel("Trend", plotOutput("suitability_trend_plot", height = "400px")),
                      bslib::nav_panel("Status", plotOutput("suitability_plot", height = "400px"))
                    )
                  ),
                  bslib::card(
...
observeEvent(input[["uploadmod-load_quant_btn"]], {
  ...
  tryCatch({
    df <- read_experiment_results(input$quant_file$datapath, vendor = input$upload_format)
    df <- create_quant_object(df, method_id = as.numeric(input$`uploadmod-method_id`))
...
################################################################################ 
  ...
  output$suitability_table <- reactable::renderReactable({
    ...
    quantobj()@suitability$suitabilitytab |>
      ...
      reactable::reactable(
        ...
        defaultColDef = reactable::colDef(minWidth = 100, format = reactable::colFormat(digits = 2)),
        filterable = FALSE,
        searchable = FALSE,
        sortable = FALSE,
  ...
  observeEvent(input$ai_suitability_btn, {
    ...
    tryCatch({
      ...
      showModal(modalDialog(title = "AI Suitability Report", 
        txtoutput,
...
observeEvent(input$exit, {
    shinyalert::shinyalert("Are you sure you want to exit?",
      ...
      callbackR = function(){ stopApp(quantobj())}
    )
  })
}
