...
run_summary.PeakRes <- function(peaks_res){
    ...
    cat(
        ...
        sprintf("Instrument: %s", peaks_res$res$sample_instrument |> unique() |> paste(collapse = ", ")),
        sprintf("first sample run time: %s %s",
            peaks_res$res$sample_createdate[1],
            peaks_res$res$sample_createtime[1]) ,
        sprintf("last sample run time: %s %s",
            peaks_res$res$sample_createdate[nrow(peaks_res$res)],
            peaks_res$res$sample_createtime[nrow(peaks_res$res)]),
        sprintf("Number of samples: %s", length(peaks_res$res$sample_vial |> unique())),
        sprintf("Number of injections: %s", length(peaks_res$res$sample_name |> unique())),
        sprintf("Number of compounds: %s", length(peaks_res$res$cmpd_name |> unique())),
        sprintf("Run injection volume(s): %s", peaks_res$res$sample_injectvolume |> unique() |> paste(collapse = ", ")),
        sep = "\n"
    )
}
...
#' @title Calculate Percision for samples tagged for suitbalility
#' @description Percision
#' @param peaks_res PeakRes object
#' @import forcats
#' @importFrom ggplot2 ggplot geom_point geom_path labs
#' @return ggplot2 object
#' @export
suitability_status <- function(peaks_res) {
    checkmate::assertClass(peaks_res, "PeakRes")



    # find sample_name with "suitability"
    peaks_res <- peaks_res$res |> filter(grepl("suitability", sample_name, ignore.case = T))
    daterun <- peaks_res$sample_createdate[1]
    descrun <- peaks_res$sample_desc[1]

    # make sure you have at least 2 samples
    if(nrow(peaks_res) < 2) stop("Need at least 2 samples for percision calculations")

    per <- c()
    # IS percision
    ISvec <- peaks_res |> distinct(sample_name, .keep_all = T) |>
            filter(sample_type == "Blank") |>
            pull(ISPEAK_area) |> rev()
    for(i in seq_along(ISvec)){
        per <- c(per,  precision(ISvec[1:i], "CV", percent = T))
  }


  df <- data.frame(a = forcats::fct_rev(factor(seq_along(ISvec))),  b = per)

  ggplot(df, aes(x=a, y=b)) + geom_point() +  geom_path(group = 1) +
  labs(y = paste0("Percision", " CV", ifelse(T, "%", "")), x ="Last N Samples",
  title = "System Suitability Percision", caption = paste("Run Date: ", daterun, " Description: ", descrun))
}
...
#' @return ggplot2 object
#' @export
plot_peak_areas.PeakRes <- function(peaks_res, normalize = TRUE,
    compounds = NULL,
    blanks = T, analytes = T, standards = T, QCs = T, type = "bar", ...){
    checkmate::assertClass(peaks_res, "PeakRes")
    ...
    filtervec <- c()
    if(blanks) filtervec <- c(filtervec, "Blank", "DoubleBlank", "ISBlank")
    if(analytes) filtervec <- c(filtervec, "Analyte")
    if(standards) filtervec <- c(filtervec, "Standard")
    if(QCs) filtervec <- c(filtervec, "QC")
    ...
    if(type == "bar"){
        res <- ggplot(y, aes(y = sample_name, x = !!sym(yname), fill = sample_type)) +
        geom_bar(stat = "identity", position = "dodge") + facet_wrap(~cmpd_name, scale = "free")

    }


    if(type == "line"){
        res <- ggplot(y, aes(x = sample_name, y = !!sym(yname),
                    group = interaction(cmpd_name, sample_type),
                    color = cmpd_name, shape = sample_type # linetype = sample_type
                    )) +
        geom_line(size = 1.1) +
        geom_point(size = 2) +
...
#' @return ggplot2 object
#' @export
plot_RT.PeakRes <- function(peaks_res, normalize = T, blanks = T, analytes = T,
    standards = T, QCs = T, facet = F, compounds = NULL){
    checkmate::assertClass(peaks_res, "PeakRes")
    ...
    y <- y |>
         mutate(sample_name = forcats::as_factor(sample_name)) |>
            filter(sample_type %in% filtervec) |>
            mutate(across(c(PEAK_startrt, PEAK_endrt, PEAK_foundrt) , ~ if_else(.x == 0, NA, .x)))
...
#' @return ggplot2 object
#' @export
percision_per_vial <- function( peaks_res ,  suitability = F){
    checkmate::assertClass(peaks_res, "PeakRes")
    ...
    y |> select(sample_name, sample_vial, sample_type, ISPEAK_area) |>
        distinct() |>
        slice_head(n = 1, by = "sample_name") |>
      mutate(ISPEAK_area = case_when(is.na(ISPEAK_area) ~ 0.01,
                                   .default = ISPEAK_area)) |>
        group_by(sample_vial, sample_type) |>
    summarize(percisio = percision(ISPEAK_area, "CV", percent = T)) |>
    ggplot(aes(x = sample_vial, y = percisio, fill = sample_type)) + geom_bar(stat = "identity", position = "dodge")
...
#' @export
area_report.PeakRes <- function(peaks_res, normalize = TRUE, blanks = T, analytes = T, standards = T, QCs = T, compounds = NULL){
    checkmate::assertClass(peaks_res, "PeakRes")
    checkmate::assertLogical(normalize)
    checkmate::assertLogical(blanks)
    checkmate::assertLogical(analytes)
