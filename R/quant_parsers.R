...
peakres_to_chromres <- function(peakres, method = NA){
  ...
  metadata_df <- lapply(split(peakres$res, peakres$res$cmpd_id), 
    \(x) x |> select("sample_id", "sample_name", "sample_type", "sample_dilutionfac",
      "sample_createdate", "sample_createtime",
      "cmpd_id", "cmpd_name",
      "PEAK_area", "ISPEAK_area", "PEAK_chromtrace") |>
      mutate(rel_response = PEAK_area / ISPEAK_area)) 
  
  metadata_df <- peakres$res |> select("sample_id", "sample_name", "sample_type", "sample_dilutionfac",
      "sample_createdate", "sample_createtime", "sample_vial", "sample_injectvolume") |>
      rename(type = "sample_type", dilution_factor = "sample_dilutionfac", 
        filename = "sample_name", sample_location = "sample_vial")  |> 
  ...
  transitions_df <- do.call(rbind, lapply(split(peakres$res, peakres$res$cmpd_id),
      \(x) x |> select("PEAK_chromtrace") |> unique())) |> 
  ...
  int_area <- peakres$res |> 
    select("sample_name", "PEAK_area", "PEAK_foundrt", "PEAK_startrt", "PEAK_endrt", 
      "PEAK_height", "PEAK_chromtrace", "cmpd_name") |>
    mutate(across(c(PEAK_area, PEAK_foundrt, PEAK_startrt, PEAK_endrt, PEAK_height), as.numeric)) |>
    ...
    rename(filename = "sample_name", area = "PEAK_area", observed_rt = "PEAK_foundrt",
      observed_peak_start = "PEAK_startrt", observed_peak_end = "PEAK_endrt", observed_peak_height = "PEAK_height") |>
    select("filename", "area", "compound_id", "observed_peak_start", "observed_peak_end", "observed_peak_height", "observed_rt") |> 
    filter(!is.na(compound_id))

  if(nrow(int_area) == 0){
    stop("No peaks found. Possibly compound name mismatch from file and database")
  }

  
  chrom_res@peaks <- rows_update(chrom_res@peaks, int_area, by = c("filename", "compound_id"))
...
.parse_tlynx_xml <- function(xmlpath, drop_prefix = FALSE){
    ...
    checkmate::assertLogical(drop_prefix)
          ...
          if(length(xml2::xml_attrs(i)) == 2) next
            ...
            for(ii in xml2::xml_children(i)){ # compounds
              if(xml2::xml_name(ii) == "COMPOUND"){
                ...
                if(drop_prefix) tmp$sample_name <-  gsub(pattern = "^.*?_" , "",  x =tmp$sample_name)
                x <- append(x, list(tmp))
              }
            }
        }
      ...
      .peak_res( dplyr::bind_rows(x) |>
            mutate(across(c(PEAK_startrt, PEAK_endrt, PEAK_foundrt, PEAK_area, ISPEAK_area, PEAK_foundscan), as.numeric)) |>
...
.parse_tlynx_csv <- function(filepath, first_cmpd){
    ...
    for(i in 1:(length(compound_rows) - 1)){ # delete first redundant row
        mylist[[i+1]] <- x[(compound_rows[i] + 1):(compound_rows[i + 1] - 1), ] |> 
            filter(row_number() != 1)
    }

    # Add the last compound data frame
    mylist[[length(compound_rows)+1]] <- x[(compound_rows[length(compound_rows)] + 1):nrow(x), ] |> 
        filter(row_number() != 1)
